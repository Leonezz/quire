...
import { AskButton, Inspector, InspectorPanel, InspectorSection, InspectorTab, InspectorTabs, ItemGroup, ItemList, ItemRow, Kbd, Segment, Segmented, Sidebar, SidebarItem, SidebarSection, Toolbar, ToolbarButton, ToolbarGroup, ToolbarTitle, type Key, type Selection } from "@read/ui";
...
export function App() {
  ...
  return (
    <div className={view === "library" ? "grid h-full grid-cols-[236px_320px_minmax(0,1fr)] grid-rows-[56px_minmax(0,1fr)] gap-3 p-3" : askOpen ? "grid h-full grid-cols-[236px_minmax(0,1fr)_360px] grid-rows-[56px_minmax(0,1fr)] gap-3 p-3" : "grid h-full grid-cols-[236px_minmax(0,1fr)] grid-rows-[56px_minmax(0,1fr)] gap-3 p-3"}>
      ...
      {view === "library" ? (
        <>
          <section className={`col-start-2 row-start-2 flex min-h-0 flex-col overflow-hidden ${panelClass}`}>
            ...
            {library.length ? (
              <ItemList aria-label="Library" items={library} selectionMode="single" selectionBehavior="replace" selectedKeys={librarySelected} onSelectionChange={setLibrarySelected} className="flex-1">
                {(item) => <ItemRow id={item.id} title={item.title} source={item.origin === "agent" ? "Agent" : item.origin === "file" ? "Local file" : hostLabel({ ...item, finalUrl: item.url })} time={timeOf(item.fetchedAt)} minutes={item.readingMinutes} state="read" signals={item.mediaType === "application/pdf" ? ["pdf"] : []}
                  {...(item.origin === "agent" ? { tag: "artifact" as const, ...(item.lineage?.length ? { detail: <span className="text-label-3">from {item.lineage.length} {item.lineage.length === 1 ? "material" : "materials"}</span> } : {}) } : item.quality.safety === "degraded_plaintext" ? { tag: "summary" as const } : {})} />}
              </ItemList>
            ) : (
          ...
          <section className="col-start-3 row-start-1 row-span-2 flex min-h-0 flex-col">
            ...
            return chosen ? (
              <ErrorBoundary key={chosen.id} label="The reader"><MaterialReader id={chosen.id} onOpenLink={openLink} onOpenMaterial={showMaterial} /></ErrorBoundary>
      ...
      {askOpen && view !== "library" ? (
        <Inspector aria-label="Inspector" selectedKey={inspectorTab} onSelectionChange={setInspectorTab}>
          ...
          <InspectorPanel id="agent"><AgentPanel context={{ kind: "library" }} onOpenMaterial={showMaterial} onOpenLink={openLink} /></InspectorPanel>
        </Inspector>
      ) : null}
