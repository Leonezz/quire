...
import { ToolbarButton } from "@read/ui";
...
export function FindBar({ root, onClose, generation }: { root: HTMLElement | null; onClose: () => void; generation: number }) {
  ...
  return (
    <div role="search" className="glass-strong absolute right-9 top-[76px] z-10 flex items-center gap-1.5 rounded-pill py-[5px] pl-3 pr-1.5">
      <SearchIcon className="size-3.5 text-label-3" />
      <input
        ref={inputRef}
        value={query}
      ...
      <ToolbarButton aria-label="Previous" isDisabled={!ranges.length} onChange={() => step(-1)} className="h-[26px] min-w-[26px]"><ChevronUp /></ToolbarButton>
      <ToolbarButton aria-label="Next" isDisabled={!ranges.length} onChange={() => step(1)} className="h-[26px] min-w-[26px]"><ChevronDown /></ToolbarButton>
      <ToolbarButton aria-label="Close" onChange={onClose} className="h-[26px] min-w-[26px]"><X /></ToolbarButton>
    </div>
  );
}
