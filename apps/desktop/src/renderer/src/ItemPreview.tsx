...
export interface PreviewAction { label: string; kbd: string; onPress: () => void; variant?: ButtonVariant }
...
export function ItemPreview({ item, actions, busy, failure, error, onOpenLink }: { item: ItemRecord; actions: PreviewAction[]; busy: boolean; failure?: string | undefined; error?: string | undefined; onOpenLink: (url: string) => void }) {
  ...
  return (
    <div key={item.id} className="panel-enter flex min-h-0 flex-1 flex-col overflow-auto">
      <div className="px-9 pt-7">
        ...
        <div className="flex flex-wrap gap-x-3 text-[12.5px] text-label-2"><span>{when}</span><span>{item.readingMinutes} min</span>{item.openedAt ? <span>opened {dayLabel(item.openedAt).toLowerCase()}</span> : null}</div>
      </div>
      <div className="flex flex-wrap items-center gap-2 px-9 py-[18px]">
        ...
        {rest.map((action) => <Button key={action.label} variant={action.variant ?? "default"} onPress={action.onPress} isDisabled={busy}>{action.label} <Kbd>{action.kbd}</Kbd></Button>)}
      ...
      <div className="grid max-w-[752px] gap-4 px-9 pb-10">
        {failure ? <InlineError title="Could not open this item." message={failure} link={item.link} onOpenLink={onOpenLink} /> : null}
