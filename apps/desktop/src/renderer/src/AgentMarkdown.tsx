...
// Streaming text is rendered as it arrives, so an unterminated fence is still shown as code.
...
export function renderInline(text: string, handlers: InlineHandlers): ReactNode[] {
  ...
  for (const match of text.matchAll(INLINE)) {
    ...
    if (citation?.[1]) {
      const id = citation[1];
      nodes.push(<CitationPill key={key} id={id} label={handlers.titleOf?.(id)} onPress={() => handlers.onOpenMaterial(id)} />);
