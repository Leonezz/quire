...
import { isMaterialViewId, nextView } from "./materialViews";
...
/**
 * The toolbar's view switch (Web · PDF · Markdown), shown when the material has more than one view. A view
 * that is not stored yet carries a down-arrow (it is fetched when chosen); one whose fetch failed carries a
 * warning with the error as its tooltip. `v` cycles through the views.
 */
        ...
        {views.map((entry) => (
          <Segment key={entry.id} id={entry.id} className="h-[24px] px-2.5 text-[12px]" aria-label={segmentName(entry)}>
            <span className="inline-flex items-center gap-1" title={entry.status === "failed" ? entry.error ?? "The last fetch failed." : undefined}>
              {entry.label}
              {entry.status === "available" ? <Icon of={ArrowDownToLine} size="sm" className="text-label-3" /> : null}
              {entry.status === "failed" ? <Icon of={TriangleAlert} size="sm" className="text-orange-text" /> : null}
            </span>
          </Segment>
        ))}
      </Segmented>
      {fetchingView ? <span role="status" className="whitespace-nowrap text-[11.5px] text-label-2">Fetching {fetchingView.label}…</span> : null}
    </span>
  );
}
...
function segmentName(entry: MaterialView): string {
  if (entry.status === "available") return `${entry.label} (fetch on demand)`;
