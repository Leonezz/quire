    ...
    <Sheet isOpen={open} onOpenChange={(next) => { if (!next && !busy) onClose(); }}>
      <SheetDialog title="Add">
        <form onSubmit={(event) => { event.preventDefault(); if (value.trim()) onSubmit(value.trim()); }} className="flex flex-col gap-4">
          <TextField autoFocus aria-label="Paste a URL" placeholder="Paste a page URL…" value={value} onChange={setValue} isDisabled={busy} {...(error ? { errorMessage: error, isInvalid: true } : {})} description="Pages are fetched once, extracted, and kept for offline reading. Or drop a Markdown, HTML or text file anywhere in the window." />
          <SheetFooter>
