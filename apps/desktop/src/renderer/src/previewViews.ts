...
// `listMaterials` here so statuses, readyViews and the record's own content follow.

const VIEWS_KEY = "read:preview-views";
const MEDIA_TYPES: Record<MaterialViewId, string> = { web: "text/html", pdf: "application/pdf", markdown: "text/markdown" };
...
/** The content of one view of the base record: the record's own for its primary, the fetched PDF otherwise. */
function contentOf(base: MaterialRecord, view: MaterialViewId, stored: StoredViews): MaterialViewContent | undefined {
...
function overlayRecord(base: MaterialRecord, stored: StoredViews): MaterialRecord {
  ...
  const content = contentOf(base, primaryView, stored);
...
export function createPreviewViews(deps: {
  ...
  const getMaterial = async (id: string) => {
    ...
    return base ? overlayRecord(base, storedOf(id)) : undefined;
  };
  ...
  return {
    ...
    getMaterialView: async (id, viewId) => {
      const base = await deps.getMaterial(id);
      return base ? contentOf(base, viewId, storedOf(id)) : undefined;
    },
    ...
    getMaterialBytes: async (id, viewId) => {
      ...
      const record = overlayRecord(base, storedOf(id));
