...
export function SourcesView({ sources, selectedId, onSelect, refresh, onOpenLink }: { sources: SourceRecord[]; selectedId: string | undefined; onSelect: (id: string | undefined) => void; refresh: () => Promise<void>; onOpenLink: (url: string) => void }) {
    ...
    <SplitGroup id="sources" aria-label="Sources" className={`min-h-0 flex-1 overflow-hidden ${panelClass}`}>
      ...
      <section className="flex h-full min-h-0 flex-col overflow-auto">
        {current ? (() => {
          ...
          return (
            <div key={current.id} className="panel-enter px-9 pb-10 pt-7">
