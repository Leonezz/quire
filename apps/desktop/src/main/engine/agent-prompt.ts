...
function materialBlock(material: MaterialRecord): string {
  ...
  const head = [`Material ${material.id}: ${material.title}`, material.byline ? `By ${material.byline}` : undefined, `URL: ${material.url}`].filter(Boolean).join("\n");
