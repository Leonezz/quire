...
function jsonFetch(status: number, body: unknown): UpdateFetch {
...
function service(overrides: Partial<UpdateServiceOptions> = {}) {
  ...
  return { updates: new UpdateService(options), states, options };
}
...
describe("UpdateService.check", () => {
  it("starts idle, passes through checking, and reports a newer release with its notes and page", async () => {
    const fetch = jsonFetch(200, [release("v0.1.0-alpha.2", { body: `  ${"x".repeat(5000)}  ` }), release("v0.1.0-alpha.1")]);
    const { updates, states } = service({ fetch });
    ...
    expect(fetch).toHaveBeenCalledWith(API_URL, { headers: { accept: "application/vnd.github+json", "user-agent": "Quire/0.1.0-alpha.1 (darwin; arm64)" } });
  });
  ...
  it("turns a missing repository, an empty list, another HTTP status, bad JSON and a network failure into error states", async () => {
    ...
    expect(await service({ fetch: vi.fn(async () => ({ ok: true, status: 200, json: async () => { throw new Error("Unexpected token"); } })) }).updates.check()).toMatchObject({ phase: "error", message: "GitHub's answer was not JSON: Unexpected token" });
