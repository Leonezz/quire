# 落地计划（2026-09-17 起）

北极星：每周被读完的材料数。范围与合同见研究仓库的 `reading-first-mvp-scope-and-plan.md`，本文件只记录"接下来按什么顺序做、做到什么算完"。每一刀独立可用、独立提交。

## 现状对照阅读合同

| 合同 | 状态（2026-09-17） |
|---|---|
| R1 一键进入、本地缓存、离线可读 | 已有（URL / 文件 → JSON 落盘） |
| R2 同一套排版 | 网页与 Markdown 已有；PDF 缺 |
| R3 阅读控制全局记忆 | 已有（缺"纸"主题） |
| R4 目录、进度、页内搜索 | 已有 |
| R5 阅读位置记忆 | HTML 已有；PDF 缺 |
| R6 来源与质量透明 | 头部有质量标签；缺"获取完整正文"动作 |
| R7 安全 | CSP 已启用；图片仍是远程加载 |
| R8 不静默降级 | 错误走 Add 面板；缺阅读页内的低质量横幅 |

## 步骤

### 第 1 刀：PDF 阅读器（R2 / R5）— 进行中
- 迁移 `PdfCanvasViewer` 到 `packages/reader-pdf`；pdf.js worker 走本地文件；渲染进程 CSP 加 `worker-src 'self' blob:`。
- 拖入 PDF / 打开 arXiv PDF 链接即读；页位置记忆；Aa 设置中主题与页宽对 PDF 同样生效。
- 验收：10 篇 arXiv PDF 打开 ≤ 1 s；关闭再开回到原页。

### 第 2 刀：评测集（博客部分）与质量档位（R6 / R8）
- `eval/`：60 个真实 URL 快照 + 每篇一个结构快照（标题、作者、日期、代码、数学、表格、脚注、图）。
- 一条命令跑全量，输出合格率与逐篇差异；不合格项落到 L2 框架 profile 或 L3 站点 profile。
- 阅读页加低质量横幅与"获取完整正文 / 打开原文"动作。
- 验收：合格率 ≥ 85%，无 P0（正文缺失、脚本外联、错位到不可读）。

### 第 3 刀：沉淀最小闭环（§2.3）
- 划线 + 一句批注，锚定到 text-quote（HTML）或页 + 区域（PDF）；Notes 面板列出并可跳转。
- 一键复制引用（引文 + 标题 + 作者 + 链接 + 位置）；标注导出 Markdown。
- 验收：关闭再开标注仍在原位；复制的引用可粘到 Obsidian 直接用。

### 第 4 刀：安全与离线收尾（R7）
- 图片本地缓存（内容寻址），无网时图片仍显示；净化 HTML 的 fixture 补齐。
- 纸色主题。
- 到此 M0 完成，打 tag。

### 第 5 刀：M1 来源与列表
- RSS / Atom 与 arXiv 类别订阅，抓取调度在主进程；Sources 显示健康度与产量。
- Inbox 三键决定（Enter / q / e）、Queue 排序与"继续阅读"、Library 全文搜索、⌘K。
- 阅读事件（opened / finished / kept）落库，北极星本地可统计。
- 验收：100 篇评测集 ≥ 90%；100 条未读 5 分钟内可决定完。

### 第 6 刀：M2 Agent
- ⌘J 面板三种位置上下文，CodexBridge + MCP 工具，移植 artifact skills；先做"阅读中解释 / 核查 / 找相关"。

## 工作方式
- 先写引擎测试再接 UI；渲染层验证走浏览器预览（`localhost:5173`，`api.ts` 的 preview 模式），主进程改动才重启桌面 app。
- 每个新交互先进 `@read/ui`，带 story 与键盘测试；a11y 对比度是阻断门。
- 范围门禁：tags、关系图、项目管理不进这几刀。

## 已完成
- 2026-09-16 `6001296` 仓库脚手架、ADR 0001、`@read/ui`。
- 2026-09-17 `c6d87a3` M0 第一刀：normalize + reader 迁移，URL 抓取即读。
- 2026-09-17 `e520754` M0 第二刀：Aa 阅读设置、⌘F 页内搜索、文件拖入、浏览器预览模式。
